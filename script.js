...
const state = {
  schema: {
    title: "Untitled Form",
    description: "A simple form created with Formify 📝✨",
    fields: []
  },
  activeId: null
};
...
function render() {
  ...
  state.schema.fields.forEach((f, idx) => {
    ...
    li.innerHTML = `
      <span>${idx + 1}. ${f.label} <small>(${f.type})</small></span>
      <span>
    ...
    li.querySelector('[data-act="up"]').onclick = (e) => {
      e.stopPropagation();
      move(f.id, -1);
    };
    li.querySelector('[data-act="down"]').onclick = (e) => {
      e.stopPropagation();
      move(f.id, 1);
    };
...
function propEditor(f) {
  return `
    ...
    <div><label>Required</label><input type="checkbox" name="required" ${
      f.required ? "checked" : ""
    }></div>
    ${
      f.type === "select"
        ? `<div><label>Options (comma)</label><input name="options" value="${(
            f.options || []
          ).join(",")}"></div>`
        : ""
    }
    ${
      ["text", "email", "number", "textarea"].includes(f.type)
        ? `
      <div><label>Min Length</label><input name="minLength" type="number" value="${
        f.minLength || ""
      }"></div>
      <div><label>Pattern (regex)</label><input name="pattern" value="${
        f.pattern || ""
      }"></div>`
        : ""
    }
...
function bindPropForm(f) {
  propForm.oninput = (e) => {
    ...
    else if (t.name === "minLength")
      f.minLength = t.value ? Number(t.value) : undefined;
    else if (t.name === "options")
      f.options = t.value.split(",").map((s) => s.trim()).filter(Boolean);
